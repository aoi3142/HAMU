...
COMMON_ARGS=(
  ...
  --output_dir "$ARTIFACT_ROOT/results/cifar/hamu"
)
...
if [[ "${USE_OPTIMIZER:-0}" == "1" ]]; then
  COMMON_ARGS+=(--use_optimizer)
fi
if [[ "${FULL_GRAD:-0}" == "1" ]]; then
  COMMON_ARGS+=(--full_grad)
fi
if [[ "${STOP_ON_STOPPING_CRITERION:-0}" == "1" ]]; then
  COMMON_ARGS+=(--stop_on_stopping_criterion)
fi
if [[ "${USE_WANDB:-1}" == "1" ]]; then
  COMMON_ARGS+=(--use_wandb --wandb_project "$WANDB_PROJECT" --wandb_run_name "${SEED}_${FORGET_CLASS}_${HARD_PROBABILITY}_${METHOD}_${PAIRING}_lr${LR}_c${CONSTRAINT}")
fi
