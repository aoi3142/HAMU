...
ARGS=(
  ...
  --output_dir "$ARTIFACT_ROOT/results/waterdrum_tofu/baselines"
)

# WANDB_PROJECT=unlearn_TOFU
WANDB_PROJECT=unlearn_TOFU_rebuttals
...
if [[ "${USE_WANDB:-1}" == "1" ]]; then
  ARGS+=(--use_wandb --wandb_project "$WANDB_PROJECT" --wandb_run_name "${SEED}_${SUBSET}_${MODEL_VARIANT}_${METHOD}_lr${LR}")
fi
